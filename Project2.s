...
# N = 33, M = 23 which is W
  ...
  invalid:
		...
		beq $s0, 0, insubstring# check for null
		beq $s0, 10, insubstring #checks for new line
		beq $s0, 44, loop_wrg #check for comma
...
next1:
	...
	inspect:
		bgt $t3,0, loop_wrg #checks to see if I have spaces or tabs between my valid characters:
		beq $s0, 9,  space #checks for tab characters
		beq $s0, 32, space #checks for  space character
		ble $s0, 47, loop_wrg # checks for ascii less than 48
		ble $s0, 57, correct # checks for integers
		ble $s0, 64, loop_wrg # checks for ascii less than 64
		ble $s0, 87, correct    # checks for my capital letters
		ble $s0, 96, loop_wrg # checks for ascii less than 96
		ble $s0, 119, correct     # checks for lowercase letters
		bge $s0, 120, loop_wrg # checks for ascii greater than 120
	...
	loop_wrg:
		# loads the bit that $t1 is pointing to
		...
		j loop_wrg #go to the beginning of loop
	...
	next2:
		bgt $t3,0,insubstring #checks for spaces or tabs in between valid characters
		bge $t4,5,insubstring #checks for  more than 4 for characters
		...
		beq $s0,44, loop_wrg #checks if the next bit is a comma
	...
	num:
		...
		beq $t4, 0, add_str    # if no hay chars exponent is zero
		li $t8, 33
		j exponent
	...
	exponent:
		...
		ble $t9, 1, add_str    #if exp  is 1 dont multiply by itself
	...
	print:
		...
		beq $s7,-1, error #checks to see if element is invalid
	...
	error:
		li $v0, 4
		la $a0, mistake #prints not valid symbol
		syscall
